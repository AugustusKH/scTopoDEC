...
# Modern Scanpy import
import scanpy as sc
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import scale

# Added modern TF2 Sequence inheritance for safe data generation
...
class AnnSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # return an (X, Y) pair
        return {'counts': batch, 'size_factors': batch_sf}, batch
...
def normalize(adata, filter_min_counts=True, size_factors=True, normalize_input=True, logtrans_input=True):
    ...
    if size_factors:
        # Safely calculate n_counts for both sparse and dense matrices
        adata.obs['n_counts'] = np.ravel(adata.X.sum(axis=1))
        adata.obs['size_factors'] = adata.obs['n_counts'] / np.median(adata.obs['n_counts'])
        # Replaced deprecated normalize_per_cell with modern normalize_total
