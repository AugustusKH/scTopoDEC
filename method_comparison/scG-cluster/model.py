...
from module import EncoderG, DecoderX, DecoderA
...
class scG(tf.keras.Model):
    # FIXED: Added n_clusters as a dynamic parameter (defaulting to 4 for safety)
    def __init__(self, raw_X, X, size_factor, model_pth, G_adj, G_adj_n, L_adj, L_adj_n, n_clusters=4,
                 hidden_dim=256, latent_dim=128, dec_dim=None, adj_dim=32):
    ...
    def train(self, y, epochs=300, lr=0.0009, W_a=0.01, W_x=1.0, W_c=1.0, info_step=1, n_update=20,
        ...
        best_loss = math.inf
        # FIXED: Initialize safely with zeros, not an empty list
        y_last_pred = np.zeros_like(y)
        loss_count = 0
        ...
        for epoch in range(0, epochs):
            ...
            if epoch % n_update == 0:
                ...
                # We extract f1 out of the tuple returned by updated cluster_acc
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                # Handle delta_y check securely
                if epoch == 0:
                     delta_y = 1.0 # Force no early exit on epoch 0
                else:
                     # Only check accuracy delta between current and last prediction array
                     acc_delta, _ = cluster_acc(y_last_pred, y_pred)
                     delta_y = 1 - acc_delta
