...
class AutoEncoder(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.fc2 = nn.Linear(in_features=10, out_features=1152)

        # Decoder Convolutions (Ghost layers 4, 5, 6 removed!)
    ...
    def forward(self, x):
        batch_size = x.size(0) # Safer than relying strictly on -1 in views
        ...
        x = self.fc2(u)
        x = torch.relu(x)
        x = x.view(batch_size, 128, 3, 3) # Unflatten back to 2D
        ...
        # Flatten back to 1D array of 784 genes for the output
        x1 = x.view(batch_size, 784)
