...
# FIXED 1: Safely combine paths so it works whether we include a slash or not
filename = os.path.join(args.filepath, args.filename)
save_path = os.path.join(args.filepath, args.resultfile)
...
if filename[-3:] == "csv":
    ...
    # FIXED 2: Replaced deprecated delim_whitespace with sep='\s+'
    adata = pd.read_csv(filename, header=0, index_col=0, sep='\s+')
    print(adata.shape)
    adata=adata.T # transpose adata if it is gene*cell
    data=sc.AnnData(adata, dtype=np.float32)
    ...
    raise ValueError("Unsupported file format. Please use .csv, .txt, or .mtx")
