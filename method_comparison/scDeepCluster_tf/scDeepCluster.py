"""
Implementation of scDeepCluster for scRNA-seq data (Modernized for TF2/Python 3)
"""
...
from scipy.optimize import linear_sum_assignment # Replaces deprecated linear_assignment

import scanpy as sc # Replaces scanpy.api
...
def autoencoder(dims, noise_sd=0, init='glorot_uniform', act='relu'):
    ...
    # hidden layer
    h = Dense(dims[-1], kernel_initializer=init, name='encoder_hidden')(h)  
...
class ClusteringLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        # TF2 requires explicit shape argument
...
class SCDeepCluster(object):
    def __init__(self, dims, n_clusters=10, noise_sd=0, alpha=1.0, ridge=0, debug=False):
        ...
        for i in range(1, len(ae_layers)):
            ...
            if "encoder_hidden" in ae_layers[i].name:
                break
    ...
    def fit(self, x_counts, sf, y, raw_counts, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140,
        ...
        if not self.pretrained and ae_weights is None:
            ...
            self.pretrain(x=[x_counts, sf], y=raw_counts, batch_size=batch_size)
            self.pretrained = True
...
if __name__ == "__main__":
    ...
    # Added 'r' mode for safe reading in modern h5py
    ...
    if args.ae_weights is None:
        scDeepCluster.pretrain(x=[adata.X, adata.obs['size_factors'].values], y=adata.raw.X, batch_size=args.batch_size, epochs=args.pretrain_epochs, optimizer=optimizer1, ae_file=args.ae_weight_file)

    # begin clustering
    scDeepCluster.fit(x_counts=adata.X, sf=adata.obs['size_factors'].values, y=y, raw_counts=adata.raw.X, batch_size=args.batch_size, tol=args.tol, maxiter=args.maxiter,
