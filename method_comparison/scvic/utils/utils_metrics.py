...
from scipy.stats import entropy  # FIXED: Removed the deprecated and removed 'kde' module
...
def log_zinb_positive(x, mu, theta, pi, eps=1e-8):
    ...
    if theta.ndimension() == 1:
        theta = theta.view(1, theta.size(0))  # Reshape for broadcasting
...
def cluster_acc(y_true, y_pred):
    """
    Calculate clustering accuracy using optimal Hungarian matching.
    """
...
def entropy_batch_mixing(
    ...
    """
    Calculates batch mixing score using local neighbor batch frequency relative entropy.
    """
    def counts_frequency(hist_data):
        counts = np.zeros(n_batches)
        for batch in range(n_batches):
            counts[batch] = np.sum(hist_data == batch)
...
def getNClusters(data, n_cluster, range_min=0, range_max=4, max_steps=20, verbose=False):
    """
    Binary search optimization to find the exact Louvain resolution parameter 
    yielding the target number of clusters.
    """
