...
class CPosterior(Posterior):
    ...
    def get_latent(self, give_mean: Optional[bool] = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        :param give_mean: z mean or z sample
        :return: two np.ndarrays, latent and labels_pred
        """
        latent = []
        labels_pred = []
        for tensors in self:
        ...
        return (
            np.array(torch.cat(latent, dim=0)),
            np.array(torch.cat(labels_pred, dim=0), dtype=int)
        )

    @torch.no_grad()
    def elbo(self) -> float:
        """Returns the Evidence Lower Bound associated to the object."""
        elbo = float(self.compute_elbo())
    ...
    def compute_elbo(self) -> float:
        ...
        # Ensure kl_divergence_global is a standard float element
        if isinstance(kl_divergence_global, torch.Tensor):
            elbo += kl_divergence_global.item()
        else:
            elbo += kl_divergence_global
            
        return float(elbo / n_samples)

    @torch.no_grad()
    def entropy_batch_mixing(self, **kwargs) -> Optional[float]:
        ...
        if self.gene_dataset.n_batches > 1:
            ...
            be_score = float(entropy_batch_mixing(latent, batches, self.gene_dataset.n_batches, **kwargs))
    ...
    @torch.no_grad()
    def imputation_list(self, n_samples: int = 1, situation: str = "corrupted") -> Tuple[List, List]:
        ...
        if situation == "corrupted":
            for tensors, corrupted_tensors in zip(
                ...
                if indices_dropout.size(0) != 0:
                    ...
                    original_list += [
                        original[cells_index == k] for k in range(actual_batch_size)
                    ]
                    imputed_list += [
                        imputed[cells_index == k] for k in range(actual_batch_size)
                    ]
                else:
                    original_list = []
                    imputed_list = []
    ...
    def imputation_score(
            self, original_list: Optional[List] = None, imputed_list: Optional[List] = None, n_samples: int = 1,
            situation: str = "corrupted"
    ) -> float:
        ...
        if len(original_list) == 0 and len(imputed_list) == 0:
        ...
        if len(original_list_concat) == 0 and len(imputed_list_concat) == 0:
            logger.info("No difference between corrupted dataset and uncorrupted dataset")
            return 0.0
        else:
            return float(np.median(np.abs(original_list_concat - imputed_list_concat)))
    ...
    def imputation_benchmark(
        ...
        for original, imputed in zip(original_list, imputed_list):
            ...
            imputation_cells += [
                np.median(np.abs(original - imputed)) if has_imputation else 0.0
            ]
        mean_score = float(np.mean(imputation_cells))
    ...
    def dropout_removal(self) -> "CPosterior":
        return self.update(
            {"collate_fn": self.gene_dataset.collate_fn_builder(dropout_removal=True)}
        )

    def dropout_not_removal(self) -> "CPosterior":
    ...
    def bayes_factors(
        ...
        if batch_sampler == "prior":
            ...
            for batch in batchid:
                counts[batch] = np.sum(self.gene_dataset.batch_indices == batch)
        ...
        if cluster_sampler == "posterior":
            ...
            # Safety wrapper if pi becomes a 0D array
            if p_cluster.ndim == 0:
        ...
        for label in np.arange(self.gene_dataset.n_labels):
            n_samples_total = int(n_samples / p_cluster[label])
    ...
    @torch.no_grad()
    def calibration(self, transform_batch: Union[None, int, str] = "max", n_samples: int = 1) -> np.ndarray:
        if transform_batch == "max":
            batchid = range(self.gene_dataset.n_batches)
            counts = np.zeros(self.gene_dataset.n_batches)
            for batch in batchid:
                counts[batch] = np.sum(self.gene_dataset.batch_indices == batch)
            transform_batch = int(counts.argmax())
        ...
        return np.array(torch.cat(calibration, dim=0))

    @torch.no_grad()
    def registration(self, transform_batch: Union[None, int, str] = "max", n_samples: int = 1) -> np.ndarray:
        if transform_batch == "max":
            batchid = range(self.gene_dataset.n_batches)
            counts = np.zeros(self.gene_dataset.n_batches)
            for batch in batchid:
        ...
        return np.array(torch.cat(denoised_scale, dim=0))
...
class CTrainer(UnsupervisedTrainer):
    ...
    def train(self, n_epochs=800, lr=1e-2, params=None, **extras_kwargs):
        super().train(
            n_epochs=n_epochs,
            lr=lr,
            params=params,
        ...
        if not self.benchmark and tol >= 0 and tol_start <= self.epoch + 2:
            ...
            if self.clustering is not None:
                ...
                if delta_clustering < tol:
                    logger.info(
                        "Tolerance achieved : ratio of clustering labels changed under " + str(tol)
                    )
    ...
    def on_epoch_begin(self):
        if self.n_epochs_pre_train == self.epoch:
            ...
            if not self.model.n_labels:
                ...
                # FIXED: Handled louvain package deprecations cleanly
                sc.tl.louvain(data, resolution=self.model.resolution)
                self.model.n_labels = len(np.unique(data.obs['louvain']))

            GMM = GaussianMixture(n_components=self.model.n_labels).fit(z.cpu().detach().numpy())
            mean_ = torch.from_numpy(GMM.means_).to(torch.float32)
            weights_ = torch.from_numpy(GMM.weights_).to(torch.float32)
            
            # FIXED: Universal device mapping logic replaces explicit .cuda() calls
            target_device = self.model.mu.device if hasattr(self.model, "mu") and self.model.mu is not None else ("cuda" if self.use_cuda else "cpu")
            self.model.mu = mean_.to(target_device)
            self.model.pi = weights_.to(target_device)
            ...
            with torch.set_grad_enabled(False):
                ...
                for name, posterior in self._posteriors.items():
                    ...
                    if posterior.nb_cells < 5:
                        logger.debug(message + " is too small to track metrics (<5 samples)")
                        continue
