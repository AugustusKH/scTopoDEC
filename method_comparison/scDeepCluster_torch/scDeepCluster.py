...
class scDeepCluster(nn.Module):
    ...
    def forwardAE(self, x):
        # BUG FIX: Only add Gaussian noise during training phase
    ...
    def forward(self, x):
        # Forward pass returning full clustering outputs
    ...
    def encodeBatch(self, X, batch_size=256):
        ...
        # BUG FIX: Wrap in no_grad to prevent massive memory leaks during evaluation
        with torch.no_grad():
...
#            acc = np.round(cluster_acc(y, self.y_pred), 5)
        ...
        delta_label = 1.0 # Initialize safely to prevent UnboundLocalError
...
#                    final_acc = acc = np.round(cluster_acc(y, self.y_pred), 5)
            ...
            for batch_idx in range(num_batch):
                ...
                inputs = xbatch.to(self.device).double()
                rawinputs = xrawbatch.to(self.device).double()
                sfinputs = sfbatch.to(self.device).double()
                target = pbatch.to(self.device).double()
