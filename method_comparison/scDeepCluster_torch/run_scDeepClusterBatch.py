...
from single_cell_tools import cluster_acc, geneSelection
...
if __name__ == "__main__":
    ...
    print("Batch Matrix Shape:", b.shape)
...
#    print("median of gene sd: %.5f" % x_sd_median)
    ...
    if args.ae_weights is None:
        model.pretrain_autoencoder(X=adata.X, B=b, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, 
                                batch_size=args.batch_size, epochs=args.pretrain_epochs, ae_weights=args.ae_weight_file)
    else:
        if os.path.isfile(args.ae_weights):
            print("==> loading checkpoint '{}'".format(args.ae_weights))
            checkpoint = torch.load(args.ae_weights, map_location=args.device)
    ...
    if args.n_clusters > 0:
        y_pred, _, _, _, _ = model.fit(X=adata.X, B=b, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, n_clusters=args.n_clusters, init_centroid=None, 
        ...
        y_pred, _, _, _, _ = model.fit(X=adata.X, B=b, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, n_clusters=n_clusters, init_centroid=cluster_centers, 
