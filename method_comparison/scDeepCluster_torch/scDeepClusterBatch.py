...
class scDeepClusterBatch(nn.Module):
    ...
    def forwardAE(self, x, b):
        # BUG FIX: Only apply Gaussian noise during training phase
        if self.training and self.sigma > 0:
            noisy_x = x + torch.randn_like(x) * self.sigma
        else:
            noisy_x = x
            
        h = self.encoder(torch.cat([noisy_x, b], dim=-1))
        z = self._enc_mu(h)
        h_dec = self.decoder(torch.cat([z, b], dim=-1))
        _mean = self._dec_mean(h_dec)
        _disp = self._dec_disp(h_dec)
        _pi = self._dec_pi(h_dec)

        # Soft assignments always use the uncorrupted representation
        h0 = self.encoder(torch.cat([x, b], dim=-1))
        z0 = self._enc_mu(h0)
        return z0, _mean, _disp, _pi

    def forward(self, x, b):
        z0, _mean, _disp, _pi = self.forwardAE(x, b)
    ...
    def encodeBatch(self, X, B, batch_size=256):
        ...
        # BUG FIX: Added no_grad() to prevent massive evaluation memory leaks
        with torch.no_grad():
    ...
    def pretrain_autoencoder(self, X, B, X_raw, size_factor, batch_size=256, lr=0.001, epochs=400, ae_save=True, ae_weights='AE_weights.pth.tar'):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (x_batch, b_batch, x_raw_batch, sf_batch) in enumerate(dataloader):
                x_tensor = x_batch.to(self.device).double()
                b_tensor = b_batch.to(self.device).double()
                x_raw_tensor = x_raw_batch.to(self.device).double()
                sf_tensor = sf_batch.to(self.device).double()
