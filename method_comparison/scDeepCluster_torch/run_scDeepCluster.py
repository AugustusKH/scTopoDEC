...
#    print("median of gene sd: %.5f" % x_sd_median)
    ...
    if args.ae_weights is None:
        model.pretrain_autoencoder(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, 
    ...
    if args.n_clusters > 0:
        y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, n_clusters=args.n_clusters, init_centroid=None, 
        ...
        y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs['size_factors'].values, n_clusters=n_clusters, init_centroid=cluster_centers, 
